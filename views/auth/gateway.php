<?php
/**
 * views/auth/gateway.php
 */
$users = UserModel::getAll();
?>
<div id="gatewayOverlay" class="<?php echo Auth::check() ? 'hidden' : ''; ?> fixed inset-0 z-[100] bg-slate-950 flex flex-col items-center justify-center p-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-md w-full mb-6">
        <!-- Misafir Butonu -->
        <button onclick="Portal.loginGuest()" class="group flex flex-col items-center justify-center p-5 rounded-3xl border border-slate-800 bg-slate-900/50 hover:bg-slate-800/80 hover:border-slate-700 transition-all cursor-pointer">
        ...
        <div class="relative group">
            <button onclick="Portal.promptAdminLogin()" class="w-full flex flex-col items-center justify-center p-5 rounded-3xl border border-purple-900/40 bg-purple-950/20 hover:bg-purple-900/40 hover:border-purple-600/50 transition-all cursor-pointer">
    ...
    <div class="max-w-md w-full space-y-2 border-t border-slate-800/80 pt-4">
        <div class="flex items-center justify-between px-1 mb-2">
            <span class="text-xs font-semibold text-slate-400 flex items-center gap-1.5">
                <i data-lucide="users" class="w-3.5 h-3.5 text-blue-400"></i> Tanımlı Kullanıcılar
            </span>
        ...
        <div id="gatewayUserList" class="space-y-2 max-h-48 overflow-y-auto pr-1">
            ...
            <?php else: ?>
                <?php foreach ($users as $u): ?>
                    <?php if ($u['id'] === 'usr_admin') continue; ?>
                    <button type="button" onclick="Portal.loginUser('<?php echo htmlspecialchars($u['id']); ?>', <?php echo !empty($u['pin']) ? 'true' : 'false'; ?>)" 
                        ...
                        <div class="flex items-center gap-3 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="text-[10px] text-slate-500"><?php echo count($u['assigned_modules'] ?? []); ?> Modül Tanımlı</div>
                            </div>
                        </div>
