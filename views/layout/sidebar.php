<?php
/**
 * views/layout/sidebar.php — Sadeleştirilmiş Çekirdek Menü
 */
        ...
        <div class="flex items-center space-x-3 px-2">
            ...
            <div class="min-w-0">
                <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-[9px] px-1.5 py-0.2 rounded-md bg-blue-500/20 text-blue-300 font-bold border border-blue-500/30">DEV</span>
                </div>
                <p class="text-[10px] text-slate-400 truncate">Özelleştirilebilir Portal</p>
            </div>
        </div>

        <!-- Menü Öğeleri -->
        <nav class="space-y-1.5">
            <button onclick="Portal.switchTab('dashboard')" id="nav-btn-dashboard" class="nav-item active-nav w-full flex items-center gap-3 px-3.5 py-2.5 rounded-2xl text-xs font-semibold text-blue-400 bg-blue-600/15 border border-blue-500/30 transition-all cursor-pointer">
                <i data-lucide="layout-dashboard" class="w-4 h-4 flex-shrink-0"></i>
                <span class="truncate">Ana Sayfa</span>
            </button>

            <?php if (($currentUser['role'] ?? '') === 'ADMIN'): ?>
            <button onclick="Portal.switchTab('admin')" id="nav-btn-admin" class="nav-item w-full flex items-center gap-3 px-3.5 py-2.5 rounded-2xl text-xs font-semibold text-slate-400 hover:text-white hover:bg-slate-800/60 transition-all cursor-pointer">
                <i data-lucide="terminal" class="w-4 h-4 flex-shrink-0 text-purple-400"></i>
                <span class="truncate">Geliştirici & Yönetim</span>
            </button>
            <?php endif; ?>
        </nav>
    </div>
