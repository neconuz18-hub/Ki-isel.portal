/**
 * Personal OS Enterprise v4.4 — Ultra Solid, Complete Modules & Zero Syntax Flaws
 */
window.Portal = {
  version: '4.4.0',
  ...
  // Tüm Sistem Modülleri (Sidebar Menü Havuzu)
  modules: [
    { id: 'dashboard', title: 'Ana Sayfa (OS)', icon: 'layout-dashboard', badge: 'Canlı', color: 'blue' },
    { id: 'tasks', title: 'Eylem & Görevler', icon: 'list-todo', badge: 'Öncelik', color: 'indigo' },
    { id: 'notes', title: 'Zihin Notları (Notion)', icon: 'book-open', badge: 'Doküman', color: 'amber' },
    { id: 'finance', title: 'Borsa & Portföy (Midas)', icon: 'trending-up', badge: 'Piyasa', color: 'emerald' },
    { id: 'routines', title: 'Rutinler & Alışkanlık', icon: 'repeat', badge: 'Günlük', color: 'purple' },
    { id: 'reminders', title: 'Hatırlatıcılar & Alarm', icon: 'bell', badge: 'Zamanlı', color: 'rose' },
    { id: 'admin', title: 'Geliştirici & Ayarlar', icon: 'settings', badge: 'Sistem', color: 'slate' }
  ],
  ...
  init() {
    ...
    this.safeExec('Clock & Weather', () => this.initClock());
  ...
  safeExec(name, fn) {
    try { 
      ...
      console.error(`[Modül Hatası - ${name}]:`, e); 
    }
  },
  ...
  switchTab(tabId) {
    ...
    if (target) {
      ...
      // Eğer spesifik tab yoksa dashboard'a dön
  ...
  initDailyBriefing() {
    ...
    const bel = document.getElementById('briefingTimeBadge'); 
    if (bel) bel.textContent = b;
  },

  toggleFocusTimer() { 
    this.focusTimerState.isRunning ? this.pauseFocusTimer() : this.startFocusTimer(); 
  },
  ...
  completeFocusSprint() { 
    ...
    this.toast('25 Dakikalık Derin Odak Tamamlandı! 🏆', 'success'); 
  },
  ...
  stopGammaAudio() {
    try {
      if (this.focusTimerState.audioNodes) {
        this.focusTimerState.audioNodes.o1.stop(); 
        this.focusTimerState.audioNodes.o2.stop();
  ...
  // --- QUICK CAPTURE (IŞIK HIZINDA YAKALA) ---
  handleQuickCapture(event) {
    if (event.key === 'Enter') {
      ...
      if (isNote) {
        ...
        this.saveLocalNotes(n); 
        this.loadNotes(); 
        this.toast('Doküman eklendi! 📝', 'success');
      } else {
        ...
        this.loadTasks(); 
        this.toast('Eylem eklendi! ✓', 'success');
      }
      input.value = ''; 
      this.playAudioFeedback('complete');
    }
  ...
  // --- TASKS (GÖREVLER & STRATEJİ GRUPLARI) ---
  getLocalTasks() {
    try {
    ...
    const initialTasks = [
      { 
        id: 'tg_1', 
        title: 'Günün Kritik Hedefleri', 
        iconType: 'pin', 
        items: [
          { text: 'Finans ve portföy durumunu incele', done: true },
          { text: 'Yeni strateji notlarını gözden geçir', done: false },
          { text: 'Deep Work odaklanma sprintini tamamla', done: false }
        ] 
      },
      {
        id: 'tg_2',
        title: 'Haftalık Proje Gelişimi',
        iconType: 'list',
        items: [
          { text: 'Midas hisse filtrelerini optimize et', done: false },
          { text: 'Kişisel OS yedeklerini kontrol et', done: true }
        ]
      }
    ];
    this.safeSetItem('portal_tasks_data', JSON.stringify(initialTasks)); 
    return initialTasks;
  },

  saveLocalTasks(t) { 
    this.safeSetItem('portal_tasks_data', JSON.stringify(t)); 
    this.updateHUD(); 
  },
  ...
  loadTasks() {
    ...
    if (bdg) bdg.textContent = `${tc} Eylem`;
    
    if (!c) return;
    
    c.innerHTML = t.map(g => {
      const itms = g.items || []; 
      const dcnt = itms.filter(i => i.done).length; 
      const tcnt = itms.length;
      ...
      const itemsHtml = itms.map((item, idx) => `
        <div onclick="Portal.toggleTaskItem('${g.id}', ${idx})" class="flex items-center justify-between gap-3 p-2.5 rounded-xl hover:bg-slate-800/80 transition cursor-pointer group">
          <div class="flex items-center gap-2.5">
            <div class="w-4 h-4 rounded-md border flex items-center justify-center text-[10px] font-black ${item.done ? 'bg-emerald-500 border-emerald-500 text-slate-900' : 'border-slate-600 group-hover:border-blue-400'}">
              ${item.done ? '✓' : ''}
            </div>
            <span class="text-xs ${item.done ? 'line-through text-slate-500' : 'text-slate-200'}">${this.escapeHtml(item.text)}</span>
          </div>
          <button onclick="event.stopPropagation(); Portal.deleteTaskItem('${g.id}', ${idx})" class="opacity-0 group-hover:opacity-100 text-rose-500 hover:text-rose-400 p-1">
        ...
        <div class="rounded-2xl bg-slate-900 border border-slate-800 overflow-hidden shadow-sm">
          <div onclick="Portal.toggleTaskGroup('${g.id}')" class="p-3.5 flex justify-between cursor-pointer hover:bg-slate-850 transition">
            <div class="flex items-center gap-2.5">
              ...
              <span class="font-bold text-xs text-white uppercase">${this.escapeHtml(g.title)}</span>
            </div>
            <div class="flex items-center gap-2 text-xs font-mono text-slate-400">
              <span class="px-2 py-0.5 rounded-md bg-slate-800 text-[10px]">${dcnt}/${tcnt}</span>
          ...
          <div class="${isOpen ? 'block' : 'hidden'} p-3 pt-0 border-t border-slate-800/80 bg-slate-900/40">
            ...
            <input 
              type="text" 
              onkeydown="Portal.addTaskToGroup('${g.id}', this, event)" 
              placeholder="+ Yeni eylem yazıp Enter'a basın..." 
              class="w-full mt-2 px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500"
            >
          </div>
        </div>
      `;
  ...
  toggleTaskGroup(id) { 
    this.openTaskGroups[id] = !this.openTaskGroups[id]; 
    this.loadTasks(); 
  },
  ...
  toggleTaskItem(gId, idx) { 
    ...
    if (grp && grp.items && grp.items[idx]) {
      grp.items[idx].done = !grp.items[idx].done; 
      this.saveLocalTasks(t); 
      this.loadTasks(); 
      this.playAudioFeedback('click');
    }
  ...
  addTaskToGroup(gId, inp, e) {
    if (e.key === 'Enter' && inp.value.trim()) {
      ...
      const grp = t.find(g => g.id === gId);
      if (grp) {
        grp.items.push({ text: inp.value.trim(), done: false });
        this.saveLocalTasks(t); 
        inp.value = ''; 
        this.loadTasks(); 
        this.playAudioFeedback('click');
  ...
  handleCreateTaskGroup(e) {
    ...
    let t = this.getLocalTasks();
    let items = [];
    if (first && first.trim()) items.push({ text: first.trim(), done: false });
    t.unshift({ id: 'tg_' + Date.now(), title: title, iconType: 'list', items: items });
    this.saveLocalTasks(t); 
    this.loadTasks(); 
    this.closeModal('newTaskModal'); 
    this.toast('Grup başarıyla oluşturuldu! ✨', 'success');
  },
  ...
  // --- NOTES (ZİHİN & NOTOSFER ÜSSÜ) ---
  getLocalNotes() {
    try { 
      const s = localStorage.getItem('portal_notion_notes'); 
      if (s) {
        const parsed = JSON.parse(s);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
      }
    ...
    const initialNotes = [
      { 
        id: 'n1', 
        title: '2026 Strateji & Hedefler', 
        content: 'Yüksek odaklı projeler, borsa portföy büyümesi ve kişisel zaman yönetimi.', 
        icon: '🚀', 
        pinned: 1, 
        updated_at: new Date().toISOString() 
      },
      { 
        id: 'n2', 
        title: 'Borsa & Yatırım Prensipleri', 
        content: 'BIST 100 hisselerinde kademeli alım ve portföy risk dağılımı.', 
        icon: '📊', 
        pinned: 1, 
        updated_at: new Date().toISOString() 
      },
      { 
        id: 'n3', 
        title: 'Önemli Fikirler & İlham', 
        content: 'Her gün 25 dakikalık en az 4 derin çalışma (Deep Work) bloğu tamamlanmalı.', 
        icon: '💡', 
        pinned: 0, 
        updated_at: new Date().toISOString() 
      }
    ];
    this.safeSetItem('portal_notion_notes', JSON.stringify(initialNotes)); 
    return initialNotes;
  },

  saveLocalNotes(n) { 
    this.safeSetItem('portal_notion_notes', JSON.stringify(n)); 
    this.updateHUD(); 
  },

  loadNotes() {
    let n = this.getLocalNotes();
    const countEl = document.getElementById('notesWidgetCount');
    const badgeEl = document.getElementById('notesSubBadgeCount');
    if (countEl) countEl.textContent = `${n.length} Doküman`;
    if (badgeEl) badgeEl.textContent = n.length;
    
    if (this.noteFilterTab === 'pinned') n = n.filter(x => x.pinned);
    if (this.searchQuery) {
      n = n.filter(x => 
        (x.title || '').toLowerCase().includes(this.searchQuery) || 
        (x.content || '').toLowerCase().includes(this.searchQuery)
      );
    }
    ...
    grid.innerHTML = n.map(item => `
      <div onclick="Portal.openEditNoteDrawer('${item.id}')" class="p-4 rounded-3xl bg-slate-900 border border-slate-800 hover:border-amber-500/40 cursor-pointer group flex flex-col justify-between min-h-[140px] transition-all shadow-sm">
        <div>
          <div class="flex justify-between items-start mb-2">
            <div class="flex items-center gap-2">
              ...
              <h4 class="font-bold text-sm text-white truncate w-36">${this.escapeHtml(item.title)}</h4>
            </div>
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onclick="event.stopPropagation(); Portal.quickPinNote('${item.id}')" class="p-1 text-slate-400 hover:text-amber-400" title="Sabitle">
                <i data-lucide="pin" class="w-3.5 h-3.5"></i>
              </button>
              <button onclick="event.stopPropagation(); Portal.quickDeleteNote('${item.id}')" class="p-1 text-slate-400 hover:text-rose-400" title="Sil">
        ...
        <div class="pt-2 border-t border-slate-800/40 flex items-center justify-between text-[10px] text-slate-500 font-mono">
          <span>${new Date(item.updated_at).toLocaleDateString('tr-TR')}</span>
          ${item.pinned ? '<span class="text-amber-400 font-bold">● Sabit</span>' : ''}
        </div>
      </div>
    `).join('');
  ...
  toggleWidgetSearch() {
    ...
    if (sb) { 
      ...
      if (!sb.classList.contains('hidden')) document.getElementById('noteSearchInput').focus(); 
    }
  },
  ...
  toggleSortNotes() { 
    ...
    this.loadNotes(); 
    this.toast('Sıralama güncellendi', 'info'); 
  },
  
  setNoteFilterTab(tab) {
    this.noteFilterTab = tab;
    const a = document.getElementById('filterTabActive'); 
    if (a) a.className = tab === 'active' ? 'p-1.5 rounded-lg bg-blue-600 text-white' : 'p-1.5 rounded-lg text-slate-400 hover:text-white';
    const p = document.getElementById('filterTabPinned'); 
    if (p) p.className = tab === 'pinned' ? 'p-1.5 rounded-lg bg-blue-600 text-white' : 'p-1.5 rounded-lg text-slate-400 hover:text-white';
    this.loadNotes();
  },
  
  filterNotes() { 
    this.searchQuery = (document.getElementById('noteSearchInput').value || '').toLowerCase(); 
    this.loadNotes(); 
  },
  ...
  openNewNoteDrawer() {
    ...
    document.getElementById('noteDrawerEmojiBtn').textContent = this.noteIcons[0];
  ...
  openEditNoteDrawer(id) {
    const n = this.getLocalNotes().find(x => x.id === id); 
    if (!n) return;
    ...
    this.currentIconIndex = this.noteIcons.indexOf(n.icon); 
  ...
  toggleDrawerPin() { 
    const p = document.getElementById('drawerNotePinned'); 
    p.value = p.value === '1' ? '0' : '1'; 
    this.toast(p.value === '1' ? 'Sabitlendi 📌' : 'Sabitlik kaldırıldı', 'info'); 
  },
  
  handleContentInput(el) { 
    const words = el.value.trim().split(/\s+/).filter(x => x).length;
    document.getElementById('drawerWordCount').textContent = `${words} kelime`; 
  },
  
  saveDrawerNote() {
    const id = document.getElementById('drawerNoteId').value;
    const title = document.getElementById('drawerNoteTitle').value.trim() || 'Başlıksız Doküman';
    ...
    if (id) { 
      ...
      if (idx > -1) { 
        notes[idx] = { ...notes[idx], title, content, icon, pinned, updated_at: new Date().toISOString() }; 
      } 
    } else { 
      notes.unshift({ 
        id: 'note_' + Date.now(), 
    ...
    this.toast('Doküman kaydedildi! ✓', 'success');
  },
  
  deleteDrawerNote() { 
    const id = document.getElementById('drawerNoteId').value; 
    if (id) { 
      this.quickDeleteNote(id); 
      this.closeNoteDrawer(); 
      this.toast('Doküman silindi', 'success'); 
    } 
  },

  // --- FINANCE (MIDAS BORSA & PORTFÖY) ---
  ...
  renderFinance(d) {
    const vEl = document.getElementById('financeTotalValue'); 
    if (vEl) vEl.textContent = new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(d.total_value);
    
    const cEl = document.getElementById('financeTotalCost'); 
    if (cEl) cEl.textContent = new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(d.total_cost);
    
    const pEl = document.getElementById('financeTotalProfit'); 
    if (pEl) { 
      pEl.textContent = `+${new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(d.total_profit)}`; 
      pEl.className = 'text-xl font-extrabold font-mono text-emerald-400'; 
    }
    
    const ppEl = document.getElementById('financeProfitPercent'); 
    if (ppEl) { 
      ppEl.textContent = `+%${d.total_profit_percent}`; 
      ppEl.className = 'text-xs font-mono font-bold px-2 py-0.5 rounded-md bg-emerald-500/20 text-emerald-300'; 
    }
      ...
      tb.innerHTML = d.assets.map(a => `
        <tr class="hover:bg-slate-800/80 transition-colors">
          <td class="py-3 px-2 font-bold text-white flex items-center gap-2">
            <span class="px-1.5 py-0.5 rounded bg-blue-600/20 text-blue-400 text-[10px] font-mono">${a.symbol.slice(0, 2)}</span>
            ${a.symbol}
          </td>
          ...
          <td class="py-3 px-2 text-right">
            <button onclick="Portal.toast('Varlık güncellendi', 'info')" class="text-slate-400 hover:text-rose-400 p-1">
      ...
      tkr.innerHTML = this.bistCatalog.slice(0, 3).map(m => `
        <div class="flex justify-between items-center p-2.5 rounded-2xl bg-slate-900 border border-slate-800">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-bold text-white">${m.symbol}</span>
          </div>
          <div class="flex items-center gap-2 font-mono text-xs">
            <span class="text-white font-bold">${m.price} ₺</span>
            <span class="text-[10px] font-bold ${m.change.startsWith('+') ? 'text-emerald-400' : 'text-rose-400'}">${m.change}</span>
          </div>
        </div>
      `).join('');
    }
    ...
    dd.innerHTML = m.map(s => `
      <div onclick="Portal.selectStockAsset('${s.symbol}', '${s.price}')" class="p-3 hover:bg-slate-850 cursor-pointer flex justify-between items-center">
        <div>
          ...
          <span class="text-slate-400 text-xs ml-2">${s.name}</span>
        </div>
  ...
  handleSaveAsset(e) {
    ...
    this.toast('Hisse portföye eklendi! 📈', 'success');
    this.playAudioFeedback('complete');
  },
  ...
  toast(msg, t = 'success') {
    ...
    d.className = `px-4 py-2.5 rounded-2xl text-white text-xs font-bold shadow-2xl transition-all ${t === 'success' ? 'bg-emerald-600' : 'bg-blue-600'}`; 
    d.textContent = msg;
    c.appendChild(d); 
